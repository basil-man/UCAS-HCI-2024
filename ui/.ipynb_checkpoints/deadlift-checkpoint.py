...
def test_deadlift_pose():
    ...
    # 找到头关节最高的帧
    head_height = keypoints_2d[0, :, 10, 1]  # 头部的y坐标
    highest_head_idx = np.argmin(head_height)  # y坐标最小的帧

    # 找到左手腕开始向上移动的帧
    left_wrist_height = keypoints_2d[0, :, 13, 1]  # 左手腕的y坐标
    left_wrist_move_up_idx = None
    for i in range(1, len(left_wrist_height)):
        if left_wrist_height[i] < left_wrist_height[i - 1]:
            left_wrist_move_up_idx = i
            break

    # 找到右手腕开始向上移动的帧
    right_wrist_height = keypoints_2d[0, :, 16, 1]  # 右手腕的y坐标
    right_wrist_move_up_idx = None
    for i in range(1, len(right_wrist_height)):
        if right_wrist_height[i] < right_wrist_height[i - 1]:
            right_wrist_move_up_idx = i
            break

    # 添加结果信息
    result += f"头部最高的帧为: {highest_head_idx}\n"
    if left_wrist_move_up_idx is not None:
        result += f"左手腕开始向上移动的帧为: {left_wrist_move_up_idx}\n"
    else:
        result += "左手腕没有检测到向上移动的帧。\n"

    if right_wrist_move_up_idx is not None:
        result += f"右手腕开始向上移动的帧为: {right_wrist_move_up_idx}\n"
    else:
        result += "右手腕没有检测到向上移动的帧。\n"
...
def test_spike_pose():
    ...
    larm_height = keypoints_2d[0, :, 13, 1]  # 左胳膊的y坐标
    larm_highest_idx = np.argmin(larm_height)
    horizon_idx = highest_idx
    for i in range(keypoints_2d.shape[1]):
        if (
            i > highest_idx
            and keypoints_2d[0, i - 1, 16, 1] < keypoints_2d[0, i - 1, 14, 1]  # 手腕高于肩膀
            and keypoints_2d[0, i, 16, 1] > keypoints_2d[0, i, 14, 1]  # 手腕低于肩膀
        ):
            horizon_idx = i
            break
    ...
    if hh_diff < hh_diff_mean - 3 * hh_diff_std:
        if hh_diff < 0:
            ...
            result += (
                "同时，用户在转体挥臂时肘部较低，可以将手肘拉起到接近耳朵位置，体会转体时将手肘藏到耳后再转肘的感觉。"
            )
